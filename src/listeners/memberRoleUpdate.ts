...
export default new DraverAPIListener('RoleAdded').addKey('RoleRemoved').setRun(async({ client, guild, user, data, type, ...rest }) => {
    if (!user || !client.whitelist.isWhitelisted(guild, user.id)) {
        ...
        if (!member) return SendErrorLogToOwner.process({ guild, logMessage: `Je n'ai pas pu ${method === 'add' ? 'rendre' : 'reprendre'} à ${pingUser(data.member)} (\`${data.member}\`) le rôle ${pingRole(data.role)} (\`${data.role}\`) qui lui a été ${method === 'remove' ? 'ajouté' : 'retiré'} par ${pingUser(rest.userId)} (\`${rest.userId}\`)` })
