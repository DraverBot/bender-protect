...
export default new Process('wait for confirm', async({ channel, time, content = 'cancel' }: { channel: GuildTextBasedChannel, time: number, content?: string }) => {
    return new Promise<{ canceled?: boolean; user: User | null }>(async(resolve) => {
        const collector = channel.createMessageCollector({
            filter: x => channel.client.whitelist.isWhitelisted(channel.guild, x.author.id) && x.content.toLowerCase() === content,
            time,
            max: 1
        });
