...
export default new AmethystEvent('guildMemberUpdate', async (bef, aft) => {
    ...
    const change = () => {
        ...
        return (
            compare('communicationDisabledUntil') ||
            compare('isCommunicationDisabled') ||
            compare('communicationDisabledUntilTimestamp')
        );
    };
    ...
    const reedit = async () => {
        if (bef.isCommunicationDisabled() && !aft.isCommunicationDisabled()) {
            bef.disableCommunicationUntil(bef.communicationDisabledUntil, 'Auto-mod by Bender Protect').catch(
                log4js.trace
            );
        } else {
