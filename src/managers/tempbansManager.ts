...
export class TempbansManager {
    ...
    public async tempban({
        user,
        guild,
        time,
        deleteMessageCount = 604800,
        reason = `Auto-sanction by Bender Protect`
    }: {
        user: GuildMember;
        guild: string | Guild;
        time: number;
        ...
        const res = await query(
            `INSERT INTO ${DatabaseTables.Tempban} ( guild_id, user_id, endsAt ) VALUES ('${guild_id}', '${user_id}', '${at}')`
        );
    ...
    private async checkDb() {
        await query(
            `CREATE TABLE IF NOT EXISTS ${DatabaseTables.Tempban} ( guild_id VARCHAR(255) NOT NULL, user_id VARCHAR(255) NOT NULL, endsAt VARCHAR(255) NOT NULL, id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT )`
        );
        return true;
    }
