...
export class SanctionManager {
    ...
    public getList(guildId: string): sanctionDataType<false>[] {
        return this.cache.get(guildId)?.sanctions ?? Object.keys(sanctionsData).map((x: keyof typeof sanctionsData) => ({ name: SanctionName[x], enabled: sanctionsData[x].enabled, sanction: sanctionsData[x].default, time: sanctionsData[x].time }))
    }
    public setSanction(guildId: string, sanction: { name: SanctionName, type: sanctionType, time: number; enabled: boolean }) {
    ...
    private update(guild: string) {
        return query(`UPDATE ${DatabaseTables.Sanctions} SET sanctions='${JSON.stringify(this.getList(guild))}' WHERE guild_id='${guild}'`)
    }
    private async checkDb() {
        await query(`CREATE TABLE IF NOT EXISTS ${DatabaseTables.Sanctions} ( guild_id VARCHAR(255) NOT NULL PRIMARY KEY, ${Object.keys(sanctionsData).map(x => `${x} LONGTEXT`).join(', ')} )`)
