...
export default new AmethystCommand({
    ...
    if (interaction.user.id !== interaction.guild.ownerId && client.whitelist.isWhitelisted(interaction.guild, member.id)) return interaction.reply({
        embeds: [ classic(interaction.user, { denied: true }).setTitle("Utilisateur whitelisté").setDescription(`${pingUser(member)} est whitelisté`) ]
    }).catch(log4js.trace)
    ...
    if (roles.size === 0) return interaction.editReply({
        embeds: [classic(interaction.user, { denied: true }).setTitle("Aucun rôle").setDescription(`${pingUser(member)} n'a aucun rôle de modération que je peux lui retirer`)]
    }).catch(log4js.trace)

    const res = await member.roles.remove(roles).catch(() => {});
    if (!res) return interaction.editReply({
        embeds: [ classic(interaction.user, { denied: true }).setTitle("Destitution échouée").setDescription(`Je n'ai pas pu destituer ${pingUser(member)}\nVérifiez que mon rôle se trouve au-dessus des autres et réessayez`) ]
    }).catch(log4js.trace)
    ...
    interaction.editReply({
        embeds: [ classic(interaction.user, { mod: true }).setTitle("Destitution").setDescription(`${pingUser(member)} a été destitué de ses rôles de modération`) ]
    }).catch(log4js.trace)
})