...
export default new AmethystCommand({
                        ...
                        },
                        {
                            ...
                            value:
                                !command.permissions || command.permissions?.length === 0
                                    ? 'Aucune permission'
                                    : command.permissions?.map((x) => `\`${getRolePerm(x)}\``).join(' '),
                            inline: false
                        }
                    )
            ]
    ...
    interaction
        .editReply({
            embeds: [
                classic(interaction.user, { footerText: 'client', accentColor: true })
                    .setTitle("Page d'aide")
                    .setDescription(
                        `Voici la liste de mes commandes :\n${client.chatInputCommands
                            .map((x) => `${commandName(x.options.name)} : ${x.options.description}`)
                            .join('\n')}`
                    )
                    .setFields({
                        name: "Plus d'info",
                        value: `Pour avoir plus d'informations sur une commande,utilisez la commande ${commandName(
                            'help'
                        )} en précisant la commande que vous voulez voir`
                    })
            ]
        })
        .catch(log4js.trace);
