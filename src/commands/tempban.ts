...
export default new AmethystCommand({
    ...
    if (!duration || isNaN(duration))
        return interaction.reply({ embeds: [invalidTime(interaction.user)] }).catch(log4js.trace);
    ...
    const confirmation = await confirm({
        ...
        embed: classic(interaction.user, { question: true })
            ...
            .setDescription(
                `Vous êtes sur le point de bannir ${pingUser(member)} pour ${secondsToWeeks(
                    duration,
                    true
        ...
        .send({
            embeds: [
                classic(interaction.user, { accentColor: true })
                    .setTitle('Bannissement temporaire')
                    .setDescription(
                        `Vous avez été banni de **${interaction.guild.name}** par ${
                            interaction.user.username
                        } pour la raison :\`\`\`${reason}\`\`\` pendant ${secondsToWeeks(duration, true)}`
                    )
            ]
        })
        .catch(log4js.trace);
    await client.tempbans
        .tempban({
            user: member,
    ...
    interaction
        .editReply({
            embeds: [
                classic(interaction.user, { accentColor: true })
                    .setTitle('Bannissement temporaire')
                    .setDescription(`${member.user.username} a été banni pendant ${secondsToWeeks(duration, true)}`)
            ]
        })
        .catch(log4js.trace);
});
