...
export default new AmethystCommand({
        ...
        if (sanction.time) {
            const time = await GetTime.process({
                ...
                embed: classic(interaction.user, { question: true })
                    .setTitle('Durée')
                    .setDescription(
                        `Sur combien de temps voulez-vous régler la durée de **${sanction.name.toLowerCase()}** ?`
                    )
            });

            if (time === 'cancel' || time === "time's up")
                return interaction.editReply({ embeds: [cancel()] }).catch(log4js.trace);

            client.sanctions.setSanction(interaction.guild.id, {
                name: eventKey,
                enabled: true,
                time,
                type: sanctionKey
        ...
        systemReply(interaction, {
            embeds: [
                classic(interaction.user, { accentColor: true })
                    ...
                    .setDescription(
                        `La sanction **${sanction.name.toLowerCase()}**${
                            sanction.time
                                ? ` pendant ${secondsToWeeks(
                                      Math.floor(
                                          (client.sanctions.getSanction(interaction.guild.id, eventKey)?.time ?? 0) /
                                              1000
                                      )
                                  )}`
                                : ''
                        } sera appliquée sur **${event.name.toLowerCase()}**`
                    )
            ]
        });
    }
    if (cmd === 'désactiver') {
        ...
        client.sanctions.setSanction(interaction.guild.id, {
            ...removeKey(client.sanctions.getSanction(interaction.guild.id, eventKey), 'sanction'),
            type: client.sanctions.getSanction(interaction.guild.id, eventKey).sanction,
            enabled: false
        });
        ...
        interaction
            .reply({
                embeds: [
                    classic(interaction.user, { accentColor: true })
                        .setTitle('Sanction désactivée')
                        .setDescription(`La sanction de **${event.name.toLowerCase()}** a été désactivée`)
                ]
            })
            .catch(log4js.trace);
    }
    if (cmd === 'activer') {
        ...
        interaction
            .reply({
                embeds: [
                    classic(interaction.user, { accentColor: true })
                        .setTitle('Sanction activée')
                        .setDescription(`La sanction de **${event.name.toLowerCase()}** a été activée`)
                ]
            })
            .catch(log4js.trace);
    }
    if (cmd === 'afficher') {
        ...
        const embed = classic(interaction.user, { accentColor: true })
            .setTitle('Sanctions')
            .setDescription(`Voici l'état des sanctions du serveur`);
        ...
        configs.forEach((conf) => {
            ...
            embed.addFields({
                ...
                value: `${sanctionTypeData[conf.sanction].name}${
                    sanctionTypeData[conf.sanction].time
                        ? ` pendant ${secondsToWeeks(Math.floor(conf.time / 1000))}`
                        : ''
                } ( ${conf.enabled ? 'activé' : 'désactivé'} )`,
                inline: count > 0
            });
        });
