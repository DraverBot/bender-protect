...
export default new AmethystCommand({
    ...
    if (!member.kickable) return interaction.reply({ embeds: [ memberNotModeratable(interaction.user) ] }).catch(log4js.trace)

    const confirmation = await confirm({
        interaction,
        user: interaction.user,
        embed: classic(interaction.user, { question: true }).setTitle("Expulsion").setDescription(`Vous êtes sur le point d'expulser ${pingUser(member)} pour la raison \`\`\`${reason}\`\`\``)
    }).catch(log4js.trace)
    ...
    await member.send({
        embeds: [ classic(interaction.user, { accentColor: true }).setTitle("Expulsion").setDescription(`Vous avez été expulsé de **${interaction.guild.name}** par ${interaction.user.username} pour la raison :\`\`\`${reason}\`\`\``) ]
    }).catch(log4js.trace)
    ...
    interaction.editReply({
        embeds: [ classic(interaction.user, { accentColor: true }).setTitle("Expulsion").setDescription(`${member.user.username} a été expulsé`) ]
    }).catch(log4js.trace)
})