...
export default new AmethystCommand({
    ...
    if (!member.bannable) return interaction.reply({ embeds: [ memberNotModeratable(interaction.user) ] }).catch(log4js.trace)

    const confirmation = await confirm({
        interaction,
        user: interaction.user,
        embed: classic(interaction.user, { question: true }).setTitle("Bannissement").setDescription(`Vous êtes sur le point de bannir ${pingUser(member)} pour la raison \`\`\`${reason}\`\`\``)
    }).catch(log4js.trace)
    ...
    await member.send({
        embeds: [ classic(interaction.user, { accentColor: true }).setTitle("Bannissement").setDescription(`Vous avez été banni de **${interaction.guild.name}** par ${interaction.user.username} pour la raison :\`\`\`${reason}\`\`\``) ]
    }).catch(log4js.trace)
    ...
    interaction.editReply({
        embeds: [ classic(interaction.user, { accentColor: true }).setTitle("Bannissement").setDescription(`${member.user.username} a été banni`) ]
    }).catch(log4js.trace)
})