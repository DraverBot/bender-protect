...
export default new AmethystCommand({
    ...
    const channel = options.getChannel('salon', true, [
        ChannelType.GuildAnnouncement,
        ChannelType.GuildForum,
        ChannelType.GuildText,
        ChannelType.PrivateThread,
            ...
            .reply({
                embeds: [
                    classic(interaction.user, { denied: true })
                        .setDescription(
                            `Vous ne pouvez pas supprimer ce salon, car c'est celui dans lequel la commande est exécutée.\nSi vous voulez nettoyer ce salon, veuillez exécuter la commande dans un autre salon`
                        )
                        .setTitle('Nettoyage impossible')
                ]
            })
            .catch(log4js.trace);
    if (!channel.deletable)
        return interaction
            .reply({
                embeds: [
                    classic(interaction.user, { denied: true })
                        .setDescription(`Je ne peux pas supprimer le salon ${pingChannel(channel)}`)
                        .setTitle('Salon non-supprimable')
                ]
            })
            .catch(log4js.trace);
    ...
    const confirmation = await confirm({
        ...
        embed: classic(interaction.user, { question: true })
            .setTitle('Nettoyage')
            .setDescription(
                `Vous êtes sur le point de supprimer le salon ${pingChannel(
                    channel
        ...
        .editReply({
            embeds: [
                classic(interaction.user, { question: true })
                    .setTitle('Nettoyage')
                    .setDescription(
                        `Le salon ${pingChannel(channel)} va être nettoyé ${displayDate(
                            Date.now() + 5000,
                            true
                        )}, sauf si une personne whitelistée annule avant (envoyez \`cancel\` dans le chat pour annuler)`
                    )
            ],
            components: []
        })
    ...
    if (nukeConfirm.canceled)
        return interaction
            .editReply({
                embeds: [
                    classic(interaction.user, { accentColor: true })
                        .setTitle('Nettoyage annulé')
                        .setDescription(`${pingUser(nukeConfirm.user)} a annulé le nettoyage`)
                ]
            })
            .catch(log4js.trace);
    ...
    const clone = await interaction.guild.channels
        .create({
    ...
    if (!clone)
        return interaction.editReply({
            embeds: [
                classic(interaction.user, { denied: true })
                    .setTitle('Nettoyage échoué')
                    .setDescription(
                        `Je n'ai pas pu cloner ${pingChannel(
                            channel
        ...
        return interaction
            .editReply({
                embeds: [
                    classic(interaction.user, { denied: true })
                        .setTitle('Nettoyage échoué')
                        .setDescription(`Je n'ai pas pu supprimer le salon ${pingChannel(channel)}`)
                ]
            })
            .catch(log4js.trace);
    }
        ...
        clone
            .send({
                embeds: [
                    classic(interaction.user, { accentColor: true })
                        .setTitle('Nettoyage')
                        .setDescription(`Salon nettoyé par ${pingUser(interaction.user)}`)
                ]
            })
            .catch(log4js.trace);
    }
        ...
        .editReply({
            embeds: [
                classic(interaction.user, { accentColor: true })
                    ...
                    .setDescription(`Le salon ${pingChannel(clone)} a été correctement nettoyé`)
            ]
        })
        .catch(log4js.trace);
});
