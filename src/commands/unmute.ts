            ...
            .reply({
                embeds: [
                    classic(interaction.user, { denied: true })
                        .setTitle('Membre non muté')
                        .setDescription(`${pingUser(member)} n'est pas muté`)
                ]
            })
            .catch(log4js.trace);
    ...
    if (!res)
        return interaction
            .editReply({
                embeds: [
                    classic(interaction.user, { denied: true })
                        .setTitle('Rétablissement échoué')
                        .setDescription(`Je n'ai pas pu démuter ${pingUser(member)}`)
                ]
            })
            .catch(log4js.trace);
        ...
        .editReply({
            embeds: [
                classic(interaction.user, { accentColor: true })
                    .setTitle('Rétablissement de la voix')
                    .setDescription(`${pingUser(member)} a été démuté`)
            ]
        })
        .catch(log4js.trace);
});
